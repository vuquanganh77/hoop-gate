...
export default function ShoesDetail() {
    ...
    const [suggested_shoes, setSuggestedShoes] = useState([]);
    ...
    const shoes = items.find((item) => item.id === Number(id));
    
    useEffect(() => {
        if (shoes?.main_url) {
            setMainImage(shoes.main_url); // Update state when shoes are loaded
        }
    }, [shoes]);

    useEffect(() => {
        if(shoes){
            dispatch(loadComments(shoes?.id)); // Fetch user details on initial load
        }
    }, [shoes]);

    if (!shoes) return <SkeletonLoading />;
    ...
    const handleFavorite = async () => {
        ...
        try {
            ...
            if (response.ok) {
                alert(`Product "${shoes?.name}" added to favorites!`);
            } else {
    ...
    const sizes_with_quantity = sizes.map((size) => {
        ...
        const matchingShoe = shoes.sizes.find((shoe) => shoe.size.toString() === size.id);
        return {
            ...size,
            quantity: matchingShoe ? matchingShoe.quantity : 0, // Add quantity attribute
        };
    });
    ...
    // if(shoes) {
    const price = shoes ? new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "VND",
    }).format(shoes.price) : "Loading...";

    var gallery = JSON.parse(shoes.gallery);
    // }
        ...
        <div>
            <div className="flex gap-7  justify-center pb-32">
                ...
                <div className="flex flex-col gap-2 ">
                    <span className="font-semibold">{shoes?.name}</span>
                    <span className="">{shoes?.brand}</span>
                ...
                <div className="flex overflow-y-auto gap-5 py-3 scrollbar-thin">
                    {
                        items.slice(1, 6).map((item) => (
                            <ProductDetail key={item.id} source={item.main_url} name={item.name} rating={item.average_rating} description={item.description} price={item.price} type="shoes" />
                        ))
                    }
                </div>
            </div>
