...
export default function AccessoriesDetail() {
    ...
    const { items, loading } = useTypedSelector((state) => state.accessories);
    const [suggested_accessories, setSuggestedAccessories] = useState([]);
    ...
    const accessories = items.find((item) => item.id === Number(id));
    
    useEffect(() => {
        if (accessories?.main_url) {
            setMainImage(accessories.main_url); // Update state when shoes are loaded
        }
    }, [accessories]);

    useEffect(() => {
        if(accessories){
            dispatch(loadComments(accessories?.id)); // Fetch user details on initial load
        }
    }, [accessories]);

    if (!accessories) return <SkeletonLoading />;
    ...
    const handleFavorite = async () => {
        ...
        try {
            ...
            if (response.ok) {
                alert(`Product "${accessories?.name}" added to favorites!`);
            } else {
    ...
    const sizes_with_quantity = sizes.map((size) => {
        ...
        const matchingAccessories = accessories.sizes.find((accessorie) => accessorie.size.toString() === size.id);
        return {
            ...size,
            quantity: matchingAccessories ? matchingAccessories.quantity : 0, // Add quantity attribute
        };
    });
    ...
    // if(accessories) {
    const price = accessories ? new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "VND",
    }).format(accessories.price) : "Loading...";

    var gallery = JSON.parse(accessories.gallery);
    // }
        ...
        <div>
            <div className="flex gap-7  justify-center pb-32">
                ...
                <div className="flex flex-col gap-2 ">
                    <span className="font-semibold">{accessories?.name}</span>
                    <span className="">{accessories?.brand}</span>
                ...
                <div className="flex overflow-y-auto gap-5 py-3 scrollbar-thin">
                    {
                        items.slice(1, 6).map((item) => (
                            <ProductDetail key={item.id} source={item.main_url} name={item.name} rating={item.average_rating} description={item.description} price={item.price} type="accessories" />
                        ))
                    }
                </div>
            </div>
